...
JNIEXPORT jobject JNICALL Java_lpsolve_LpSolve_readMps
  (JNIEnv *env, jclass cls, jstring filename, jint verbose)
{
	lprec* lp = NULL;
	jobject result = NULL;
	jclass lpsolve_cls = NULL;
	...
	lp = read_MPS((char *)c_filename, verbose);
	if (lp == NULL) {
  ...
  (JNIEnv *env, jclass cls, jstring filename, jint verbose)
{
	...
	lp = read_freeMPS((char *)c_filename, verbose);
	if (lp == NULL) {
...
cleanup:
	...
	delete (void *)c_modelname;
	delete (void *)c_dataname;
